...
def token_sar(cached_data):
    ...
    for sample_idx, likeli in enumerate(likelihoods):
        ...
        gen_token_wise_entropy = likeli['token_wise_entropy']
        for k in range(len(gen_token_wise_entropy)):
            token_wise_entropy = gen_token_wise_entropy[k].float() # get the probability
...
def get_correctness(generations, type, threshold, args):
    ...
    if 'bertscore' in type:
        ...
        predictions = pd.DataFrame(generations)["most_likely_generation"].tolist()
    ...
    return correctness,predictions
    ...
    with open(os.path.join(config.output_dir, args.run_name, 'uncertainty_estimation_results.txt'), 'w') as f:
        for method in args.methods:
            ...
            for eval in correctness_list:
                correctness = eval['correctness'][0]
                predictions = eval['correctness'][1]
                type = eval['type']
                threshold = eval['threshold']
                if (correctness == 1).all() or (correctness == 0).all():
                    continue
                else:
                    print("AUC - correctness",correctness)  # np.array: 8003个题目对应是否正确(0/1) [8002,2]
                    print("AUC - scores",scores)            # tensor: [8003,1] 全是概率，但是好像超过了1
                    ...
                    print(info)
                    outdata = {}
                    outdata["uncertainty_score"] = scores.tolist()
                    outdata["correctness"] = correctness
                    outdata["predictions"] = [pre.lstrip().split('\n')[0] for pre in predictions]
                    outdata["auc"] = auc
                    with open(os.path.join(config.output_dir, args.run_name, 'AUROC.pkl'), 'wb') as fw:
                        pickle.dump(outdata,fw)
                    with open(os.path.join(config.output_dir, args.run_name, 'uncertainty_list.txt'), 'w') as fw:
                        fw.write(str(outdata["uncertainty_score"]))
                    f.write(info + '\n')
                    f.flush()
