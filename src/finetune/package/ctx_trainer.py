...
class CTXTrainer(Trainer):
    ...
    def __init__(
        ...
        formatting_func: Optional[List[Callable]] = None,# change into list
        ...
        included_contexts:Optional[List] = ["pos_ctx"]
    ):
        if model_init_kwargs is None:
            model_init_kwargs = {}
        elif not isinstance(model, str):
            raise ValueError("You passed model_kwargs to the CXTTrainer. But your model is already instantiated.")
        ...
        if isinstance(model, str):
            warnings.warn(
                "You passed a model_id to the CXTTrainer. This will automatically create an "
        ...
        if packing and data_collator is not None and isinstance(data_collator, DataCollatorForCompletionOnlyLM):
            raise ValueError(
                "You passed a `DataCollatorForCompletionOnlyLM` to the CXTTrainer. This is not compatible with the `packing` argument."
            )

        if is_peft_available() and peft_config is not None:
            if not isinstance(peft_config, PeftConfig):
                raise ValueError(
                    "If you want to use the PeftModel, you need to pass a PeftConfig object to the CXTTrainer."
        ...
        if max_seq_length is None:
            ...
            warnings.warn(
                f"You didn't pass a `max_seq_length` argument to the CXTTrainer, this will default to {max_seq_length}"
            )
        ...
        if neftune_noise_alpha is not None and self._trainer_supports_neftune:
            ...
            warnings.warn(
                "You passed a `neftune_noise_alpha` argument to the CXTTrainer, the value you passed will override the one in the `TrainingArguments`."
            )
        ...
        if not packing:
            if dataset_text_field is None and formatting_func is None:
                raise ValueError(
                    "You passed `packing=False` to the CXTTrainer, but you didn't pass a `dataset_text_field` or `formatting_func` argument."
                )
        ...
        if tokenizer.padding_side is not None and tokenizer.padding_side != "right":
            warnings.warn(
                "You passed a tokenizer with `padding_side` not equal to `right` to the CXTTrainer. This might lead to some unexpected behaviour due to "
        ...
        if self.args.max_steps > 0 and packing:
            warnings.warn(
                "You passed `packing=True` to the CXTTrainer, and you are training your model with `max_steps` strategy. The dataset will be iterated until the `max_steps` are reached."
            )
    ...
    def _prepare_non_packed_dataloader(
        ...
        """
        input data should be divided into three parts:
        positive context, negative context(s)[list], query, response
        there are collated into dict (json) formats.
        {
            "positive context":str,
            "negative contexts":list[str]
            "query":str
            "response":str
        }
        """
        ...
        def negative_before_tokenize(negative_context, query, response):
            return {"negative_context":negative_context, "query":query, "response":response}

        def tokenize(element):
            query = element["query"]
            response = element["response"]
            #pos elem
            positive_context = element["positive context"]
            positive_before_tokenize = {"context":positive_context, "query":query, "response":response}
            positive_outputs = tokenizer(
                formatting_func(positive_before_tokenize),
            ...
            positive_inputs = {
                "input_ids": positive_outputs["input_ids"],
                "attention_mask": positive_outputs["attention_mask"],
                }
            if "neg_ctx" in self.included_contexts:
                negative_contexts = element["negative_contexts"]
                negative_outputs_list = [tokenizer(
                    formatting_func(negative_before_tokenize(negative_context_i)),
                    add_special_tokens=add_special_tokens,
                    truncation=True,
                    padding=False,
                    max_length=max_seq_length,
                    return_overflowing_tokens=False,
                    return_length=False,
                ) for negative_context_i in negative_contexts]
                negative_inputs_list = [{
                    "input_ids": negative_output_i["input_ids"],
                    "attention_mask": negative_output_i["attention_mask"],
                    } for negative_output_i in negative_outputs_list ]
            else:
                negative_inputs_list = None
            if "emp_ctx" in self.included_contexts:
                empty_context = ""
                empty_before_tokenize = {
                    "context": empty_context,
                    "query": query,
                    "response": response,
                    }
                empty_outputs = tokenizer(
                    formatting_func(empty_before_tokenize),
                    add_special_tokens=add_special_tokens,
                    truncation=True,
                    padding=False,
                    max_length=max_seq_length,
                ...
                empty_inputs = {
                    "input_ids": empty_outputs["input_ids"],
                    "attention_mask": empty_outputs["attention_mask"],
                    }
            else:
                empty_inputs = None
            

            return {"positive_inputs":positive_inputs, "negative_inputs_list": negative_inputs_list, "empty_inputs": empty_inputs}
        # def tokenize(element):
        #     outputs = tokenizer(
    ...
    def _prepare_packed_dataloader(
        ...
        if dataset_text_field is not None or formatting_func is not None:
            if tokenizer is None:
                raise ValueError("You need to pass a tokenizer when using `dataset_text_field` with `CXTTrainer`.")
            ...
            raise ValueError(
                "You need to pass a `dataset_text_field` or `formatting_func` argument to the CXTTrainer if you want to use the `ConstantLengthDataset`."
            )
